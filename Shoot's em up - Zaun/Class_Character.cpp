	...
	if (canCastSpell(spellName)) {
		if (spellName == "A Spell") {
			...
			if (!positionHistory.empty()) {
				...
				isTeleporting = false; // Assurez-vous que la t�l�portation est d�sactiv�e si l'historique est vide.
			}
		}
...
void Ekko::dash() {
	...
	// Calculer la position finale apr�s le dash
...
void Ekko::updatePositionHistory() {
	// V�rifiez si l'historique doit �tre mis � jour
	if (positionClock.getElapsedTime().asSeconds() >= 0.1f) {
		...
		// Gardez uniquement les 4 derni�res secondes
...
void Ekko::updateTeleport() {
	if (isDashing) {
		...
		float dashDuration = 0.2f; // Dur�e totale du dash en secondes
